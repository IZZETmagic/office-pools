...
| TD-13 | **Android push notifications — in progress (Expo-relay path chosen 2026-05-16).** Server-side dispatch is complete (`lib/push/expo-push.ts` posts to `exp.host/--/api/v2/push/send`; `apns.ts > dispatchPush` routes by token shape + platform column). Mobile side fetches an `ExponentPushToken[...]` via `getExpoPushTokenAsync` on Android and registers under `platform='android'`. Firebase wiring done: project `sportpool-e34a3`, Android app for `com.officepools.expo`, `google-services.json` in `mobile/`, FCM V1 service account uploaded to EAS credentials. **Remaining**: (a) first Android dev build completes (in-flight 2026-05-16, build `0dfb7deb`), (b) sideload to a physical Android device, (c) verify the same 24-push test sweep we ran on iOS, (d) confirm the `default` notification channel created by expo-notifications shows up correctly on Android 8+. **Skipped** vs the original plan: no direct FCM dispatcher — Expo's relay is the dependency. If we ever hit the 600 notifs/sec free-tier limit or want to drop the dependency, swap to a parallel `lib/push/fcm.ts` (~1 day) using the same FCM service account JSON. |
