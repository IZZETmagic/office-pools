...
- **Known limitations:** (1) dates are the **backfill date** for pre-existing badges — accurate going forward, but the *pool breakdown* is what's meaningful now; (2) **left pools** — the `badge_unlocks` RLS gates on *current* membership (`pool_id in get_user_pool_ids()`), so a user can't see their own unlocks in a pool they've left; a truly "lifetime" case wants an additive self-read policy (`user_id` = me). One-policy fix, flagged not applied.
- **Remaining:** the dedicated-page split (v1 is a *section* in the Statistics tab) and mobile parity (OTA). Still couldn't do a live-auth render check — types + data path verified.
