...
| TD-12 | Push notifications for **badges earned** and **level-ups** are not yet wired. The streak-milestone push (Phase 1) covers part of the gamification dopamine loop but badge/level pushes require a per-entry XP state-diff snapshot to detect "newly crossed" thresholds without running computeFullXPBreakdown per affected entry on every recalc (N+1 heavy). | Missing engagement loop for two of the six gamification event types. | Add `entry_xp_state` table (entry_id PK, total_xp, current_level, earned_badge_ids[]). Snapshot inside recalculatePool after the existing match-results fan-out. Diff against the snapshot to detect new badges + level crossings, fire push, then UPDATE the snapshot. Bundle with the rest of the gamification polish post-WC. |
