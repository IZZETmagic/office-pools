...
| TD-14 | Mobile app never clears the iOS app icon badge client-side. Server-side fix (May 2026) replaces the old hard-coded `badge: 1` with a per-user unread-message count via `get_user_unread_message_count` RPC — naturally drops to 0 when the user reads everything, so the next push lands the badge at 0. But the badge state only updates when a push arrives. A user who opens the app and reads messages without receiving any new pushes will still see the old badge until the next push fires. Add an `AppState` listener in `mobile/lib/usePushNotificationHandlers.ts` that calls `Notifications.setBadgeCountAsync(0)` on app foreground — ~6 lines, ships with next mobile build. | Phase 3b.0 — bundle with next mobile build |
