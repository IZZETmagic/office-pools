...
| TD-13 | **Android push notifications not implemented.** iOS push is fully wired via APNs + 24 push types across 7 categories. Android needs the parallel stack: FCM (Firebase Cloud Messaging) instead of APNs. Affects: 0 users today (Swift app is iOS-only; Expo dev clients all iOS so far), but blocks Android GA. | Android users will silently get no push notifications until this lands. | (1) Create Firebase project + add Android app for `com.officepools.expo`, download `google-services.json`, drop in `mobile/` and reference in app.json. (2) Generate FCM service account JSON, store as Vercel env `FCM_SERVICE_ACCOUNT_JSON`. (3) Configure EAS Android build profile with keystore (EAS-managed). (4) Build `lib/push/fcm.ts` parallel to `apns.ts` — JWT-signed HTTP v1 API calls per token. (5) Update `sendPushToUser`/`sendPushToUsers` to route iOS tokens → APNs, Android tokens → FCM. (6) Mobile: detect platform in `usePushTokenRegistration.ts` (currently hardcodes `'ios'`); on Android `getDevicePushTokenAsync` returns an FCM token, registers under `platform = 'android'`. (7) Test all 24 push types on Android device. **Architectural fork to decide first**: native FCM dispatcher (matches iOS pattern, ~1 day work) vs Expo's hosted push service for Android only (~half a day, but adds Expo as dependency in the critical path + 600 notifs/sec free-tier rate limit). Bundle with Phase 3b.1 Android GA. ~1.5 days dev + half-day Firebase setup. |
