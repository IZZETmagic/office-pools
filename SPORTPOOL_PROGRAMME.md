...
| **R20** | **Neither scoring engine's stored points match what its own current code computes — and they agree with each other only because both are stale.** ⚠️ **New 2026-08-14.** Migration 042 removed `knockout_*` as a *second, parallel base* in the Node engine and folded the group→knockout ratio into the stage multipliers, so `lib/scoring/core.ts:159-161` reads `group_exact_score` for every stage. **The shadow engine was never updated**: it read `knockout_*` as a base **and** applied the folded multiplier, double-counting the ratio. This was undetectable by every check in place — the stored `shadow_match_scores` rows were written *before* 042 changed the multipliers, so they held pre-042 values that matched `match_scores` exactly, and **the parity alarm compares totals, so it cannot see that a *function* would disagree with its own stored output if re-run.** Fixed in the function by **046d** (one base for every stage, matching `core.ts`); the **stored rows are a separate question and are NOT fixed** | 🟠 | **157 rows / 6 pools / 63 entries currently sit +4,884 above `match_scores`** — the 7 World Cup matches re-scored on 2026-08-14 while verifying 046. The other 97 matches (269,359 rows) still hold pre-042 values; 16 of those were already disagreeing with Node before this work. Net drift across all 286,872 rows is **+2,184** (12,833,270 → 12,835,454), so 042's fold does reproduce the old totals almost exactly — the residual is 042's own documented *"sixteen pools whose ratio differed per tier and could not be preserved exactly"*, which no single multiplier can reconcile. **Shadow is the read source for all 623 pools, so this is what members see** | **Already partly fired.** The remaining trigger is any reconciler, sweep or manual rescore touching a completed knockout match — jobid 19/20 reconcilers, or a `sweep_pending` drain. Benign only while nothing re-scores the World Cup | **Function fixed (046d, applied).** Stored rows deliberately left alone. Rollback of the pre-046 bodies: `drafts/2026-08-14_pre046_shadow_rollback.sql`. ⚠️ There is **no row-level backup** of the 7 matches' original values — a rollback restores the formula, not the numbers | ⏳ **Ryan chose to defer, 2026-08-14** — "leave it, decide later". The three options are written up in `drafts/2026-08-14_league_platform_plan.md`: (1) re-score all 104 in shadow, (2) leave it, (3) re-score both engines. Note (2) is a deferral rather than a resolution: shadow is internally inconsistent until one of the others is chosen | *League platform plan*; **R2**; *Shadow scoring engine*; Node-retirement gates |
