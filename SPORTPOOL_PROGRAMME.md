...
| **R26** | **A matchweek does not snapshot on the whistle that completes it.** ⚠️ **New 2026-09-02, measured, and it is bigger than the mode it was found in.** After all 10 fixtures of a matchweek completed **one at a time**, with `completed_fixture_count` at **10/10** and every fixture carrying a scored witness, `ranks_snapshot_at` was **still NULL** — and one further touch to any fixture set it. The cause is trigger name order on `league_fixtures`: `broadcast_league_fixtures_upd → score_league_fixture_upd → settle_league_lms_upd → trg_refresh_league_matchweek_window_upd`. The scorer attempts the snapshot **before** the refresher updates the count, so the count it reads is always one behind, and `refresh_league_matchweek_window` does **not** re-attempt it (verified) | 🟠 | **Everything hanging off `ranks_snapshot_at` is late by one write** — Showdown duel settlement, LMS round closure, the `matchweek_completed` outbox event (*"results are in"*), and the movement arrows (`previous_final_rank`). ⚠ It self-heals in production only because the feed keeps touching completed fixtures afterwards, which is luck rather than design: **a matchweek whose last fixture is never written again never snapshots**, and its Showdown duels and LMS round never settle | Every matchweek, silently. MW2 settled because a later write happened to arrive | **None.** Two candidate fixes: have `refresh_league_matchweek_window` re-attempt `league_snapshot_matchweek_ranks` after updating the counts (idempotent — it returns 0 if already snapshotted), or rename the triggers so the refresher fires first. The **first** is safer: renaming changes ordering for the scorer and the broadcast too | **New — no call made.** Found while testing 132; deliberately NOT bundled into it, because a fix here changes when four other things fire | *Update flow, mode by mode*; **132** |
