...
> the test file and read-only production queries this evening. Where production state could not be
> read from here it says so. **The rest of this section is 2026-09-02 text and has drifted — see
> *Drift*.**

**Since last update — verified**

- ✅ **Migration 135 scheduled the four league crons** — reported applied to production tonight via
  Supabase MCP. `cron.job` itself is not reachable from this session (the `cron` schema is not
  exposed to PostgREST), so the apply is **Ryan's report, not my read**. But two of the four are
  **corroborated from the data**: `league_score_events` went from a 223-event backlog to **0
  pending of 243** at **21:52–21:54 UTC**, events created `22:00:01` were claimed `22:02:01` and
  events created `22:19:52` claimed `22:20:01` (an even-minute `*/2` cadence — `league-outbox`);
  and `matchweek_opened` + `lock_reminder` rows appeared at exactly `22:00:01` (the hourly
  `league-notices`). `league-standings` (Mon 06:00) and `league-reconcile` (03:00 daily) **cannot
  have had a scheduled firing yet** — first ones due 2026-09-20 03:00 and 2026-09-21 06:00.
  Precondition in the file's own header (routes deployed, 401 not 404) holds: all four
  `/api/cron/league-*` answer **401** tonight.
- ✅ **Root cause of the missing La Liga matchweek-5 results, verified in code and data.** Providers
  publish a TBD placeholder kickoff (La Liga: Sunday 15:00Z) and move the game later. The live sync
  (`lib/integrations/apiFootball/syncLeagueFixtures.ts`) and its hourly catch-up both build the
  provider request from the **stored** kickoff, and the daily reconcile reads only kickoffs **ahead**
  of the live window (`reconcileLeagueSchedule.ts:127`, `.gt('kickoff_at', now + SYNC_WINDOW_MS)`)
  — so once the placeholder date had passed, a game played *before* it was unreachable by every arm,
  every tick, at `ok: true`. Production tonight: **235** fixtures carry a moved kickoff, **111** to an
  earlier date; **206 of them were stamped in the 21:xx UTC hour** of the hand-run
  `scripts/run-league-reconcile.ts --apply` (Ryan's figure of 202 is consistent; I cannot reproduce
  the exact split from stored columns). 35 of La Liga's 45 moved rows had the Sunday-15:00Z original.
- ✅ **The seven stranded fixtures are recovered** — all seven carry `completed_at = 2026-09-19
  22:19:46Z`: Osasuna–Espanyol, Racing–Alavés, Real Madrid–Rayo, Sevilla–Valencia, Athletic–Elche
  (all stored 13 Sep 15:00Z, played 11–12 Sep), Lazio–AC Milan (stored 13 Sep 18:45Z, played 12 Sep),
  Monaco–Lens (stored 19 Sep 15:00Z, played 18 Sep). ⚠ Six of them sat **7 days**, not 6; Monaco–Lens
  sat **1**. Standings `fetched_at` for La Liga, Serie A and Ligue 1 are all `22:19:5x` — refreshed.
  One La Liga fixture remains past-kickoff and uncompleted (`1570389`, 16 Sep) and is genuinely
  `postponed`, not stranded.
- ✅ **Commit `fc23239`** (local, **unpushed**, not deployed) — step 3b in the sync asks for a stray
  the day feed did not carry **by fixture id** (`getFixturesByIds`, `client.ts:242`;
  `syncLeagueFixtures.ts:600-643`); the run note gains `stray_by_id=fetched/asked`
  (`syncLeagueFixtures.ts:1433`); tests **V3.13–V3.15** exist and the file passes **60/60**.
  **Commit `6eb14e4`** — `scripts/run-league-sync.ts` had been dead at import since `a3c3e19`
  (2026-09-02) put `server-only` into `lib/league/season.ts`; `scripts/lib/no-server-only.cjs`
  resolves it to an empty module. Verified: the reconcile script never imported that chain, which is
  why it could run tonight while the sync script could not.

**Now**

1. **Push + deploy `fc23239`** — until then the scheduled sync runs the *old* arm and the next
   placeholder-before-kickoff game strands again, silently. Silent-wrongness rule; one provider move
   away. **R30.**
2. **Decide what the outbox does with a stale event** — the first drain **replayed 38
   `matchweek_completed` events from MW2–MW4 (oldest 19 days)** because `league-outbox/route.ts`
   has no age guard. The allowlist (`notify.ts:110`, pickem + showdown only) limited it to **7
   pools / 20 events / ≤104 emails to ≤16 distinct users**, mostly the UX test pools; *Premier League
   2026/2027 Pool* (3 members) looks real. Whether Resend delivered them is **unverified** (no send
   log table; the Vercel log was not readable from here). **R31.**
3. **Watch the first scheduled `league-reconcile` (2026-09-20 03:00 UTC) and `league-standings`
   (2026-09-21 06:00)** — they are scheduled on report, not on evidence, until they fire.

**Risk moves** — **R30** opened 🟠 (strand fix built, unpushed). **R31** opened 🟡 (stale-event
replay; incident recorded). The *"no league cron is scheduled"* finding, which underpinned the 🔴
rows at *Nothing reminds anyone to pick* and *The outbox is never drained*, is **closed** — both
rows updated below. **R25**'s "other half" (a scheduled standings refresh) has landed; row annotated,
not rewritten. **R21** is not re-levelled here, but see *Drift*.

**Drift**

- **R21 is stale in the load-bearing way.** This section and the register still say production is
  `6dc5710` (24 Aug), 225 commits behind. `origin/master` is **`a8dee7f` (12 Sep)** and the four
  league routes answer 401 in production — the deploy the register calls "not attempted" happened.
  The *current* gap is **109 commits unpushed** (tonight, including `fc23239`). A full re-audit of
  R21 and the *Ship what is built* project is owed; **not done in this pass**.
- The sentence *"`league-notices` is undeployed and therefore unschedulable"* (Pick'em season table)
  and *"`cron.job` holds no league job of any kind"* (engine audit) were true on 2026-09-02 and are
  false tonight; both annotated in place.
- Migration 135's own header says *"confirmed against production `cron.job` on 2026-09-05: 17 jobs,
  none of them a league cron"* — that was a fresh check that the 2026-09-02 text never recorded.

**Needs Ryan**

- Push `fc23239` + `6eb14e4` (and the other 107) — pushing `master` is a production deploy; only he
  does it.
- **R31:** should the outbox drop, or mark-without-sending, any notification event older than N
  hours? Today it sends. The disclosure gate is fine with *"we tell you how you did when a matchweek
  finishes"*; it is not fine with *"…three weeks later, in a batch."*
...
**What survived of them was narrower and still 🔴** — tracked as **R22**, and ✅ **fixed the same
day** (`6aa2831`). `app/api/account/delete/route.ts` deleted match scores, bonus scores, predictions,
group predictions, special predictions, player scores, entries and memberships, and only then asked
*"do you still administer a pool?"*, returning 400 if so. **The 400 came after the destruction**: a
pool admin who tried to delete their account kept the account and lost everything, everywhere,
irreversibly. The guard is now the first thing the route does. ⚠ **Two residuals stay open** — the
check-then-act is not transactional, and this door still **hard-deletes rather than retiring**, so
`retired_at` does not defend it. That second one is **Gate A2**, which asks whether account deletion
needs a different answer from the other three doors, and it is still unanswered.
...
- **R23 — the league read path is the product's cost.** A review on 2026-08-31
  ...
  listed seven fixes. ✅ **Items 1–3 landed 2026-09-02** (`6aa2831`) — the three that silently produce
  *wrong data*: the two unbounded duel reads, the Node aggregation (now **migration 130**, ✅ applied and
...
| Finding | State |
|---|---|
| **Nothing reminds anyone to pick.** The four deployed push crons contain no league code; `league-notices` is undeployed and therefore unschedulable. For a 38-week season the weekly nudge *is* the product | ✅ **CLOSED 2026-09-19** — route deployed (401), `league-notices` scheduled hourly by migration **135**; first run at `22:00:01` UTC queued `matchweek_opened` + `lock_reminder`, drained at `22:02:01`. Was 🔴 open |
| **The outbox is never drained.** 9 `matchweek_completed` events unclaimed since 25 Aug. Producer works, consumer does not exist in production ⇒ no "results are in", no cache invalidation after scoring | ✅ **CLOSED 2026-09-19** — `league-outbox` scheduled `*/2` by **135**; 0 pending of 243 at 21:54 UTC and holding. ⚠ The first drain **replayed 38 stale `matchweek_completed` events (MW2–MW4)** to 7 pickem/showdown pools — **R31**. Was 🔴 open |
...
· **Later the same day — two of the risks this pass opened were fixed** (`6aa2831`, on Ryan's
instruction). **R22** the account-delete ordering, with a negative-tested guard. **R23 items 1–3**,
the three read-path defects that silently produce wrong data: `readPoolDuels` paged,
`readMatchweekPoints` moved into SQL as **migration 130**, and the browser's second scoring engine
removed from `buildDuelTable`. Two things were found on the way — the duel recap page discarded the
error from the read every number on it derives from, and the deny-all guard could not see an RPC.
✅ **Migration 130 was then applied to production and verified against real data.** Both risks
**close on deploy, not on the commit**.
· **Three more migrations applied the same day, all proved in rolled-back transactions on the live
season before applying, all drift-checked after.** **131** — Table mode's score never responded to a
goal; a diffed, statement-level trigger pair on `league_standings` unfroze it (**R25**), and both
frozen pools moved in test, one 760 → 940. **132** — Last Man Standing settled a week late; it now
judges at each final whistle while the round still closes at matchweek end, because judging
progressively without that gate crowns a winner whose club plays Monday. **133** — **R26**, found
while testing 132 and bigger than it: a matchweek never snapshotted on the whistle that completed
it, because trigger name order puts the window refresher after the scorer, so Showdown settlement,
LMS closure, the `matchweek_completed` outbox event and the movement arrows were all late by one
write. **All four modes now match the flow Ryan set out**, and Decisions 11–13 record the three
rulings behind them.
· **M1's season contract, M2 and the mobile half of M3, 2026-09-02.** `lib/league/season.ts` caches
the season per SEASON (165.7 kB shared by 13 pools on the PL), invalidated off the sync's own
`changed` array; `GET /api/pools/:id/league` is the RN entry point; `@tanstack/react-query` is in the
mobile app with `AppState` wired to `focusManager`. ⚠ **The M3 measurement reframed that gate**:
writes on replicated tables are **96.5% two tables** — `user_presence` **63.6%** and `pool_entries`
**32.9%** — so the biggest single realtime item is **web presence** (**new R27**), which M3 as scoped
never covered, and three replicated tables have **no consumer at all** for 0.1% between them.
· **Two decisions settled and one fix applied, 2026-09-02 (later).** **Decision 11 — a league table
is a full-time table**: `/standings` is full-time, so Table mode's bonus waits for the whistle while
the leaderboard still moves on the goal; the live overlay is deferred, not rejected. **Decision 12 —
one read, two surfaces**: the league read contract is shared rather than patched per surface, because
RN cannot read league data without it and the web's 175 kB per-request season read gets fixed in the
same move. And **migration 131 applied** — a diffed, statement-level trigger pair on
`league_standings` that unfreezes Table mode (**R25** 🔴 → 🟡).
· **Engine audit, same day, against production** — Ryan asked whether every scoring engine is in
Supabase like the shadow ones and whether each moves on a goal. **Neither is yes.** Six of ten are
DB-native; the Node engine, the analytics/XP writer and the browser's bracket-picker are not. The
league is genuinely goal-driven in the database (trigger → fixture → snapshot → duels + LMS); the
World Cup is Node plus a minute-cadence reconciler. **New 🔴 R25: Table mode's score never responds
to a goal** — `league_after_standings_change` exists and is wired to nothing, so the only writer in
production is the member's own save, and two pools past their deadline are frozen. Full working under
*Scoring engines → Engine audit*.
...
**Gate M3 · Retire mobile's `postgres_changes`.** 🟡 **The big one is done in code** — `usePoolEntries`
...
| **Clients computing what the backend already knows** | the 26,770-row leaderboard that motivated the architecture rule | ✅ **FIXED 2026-09-02** (`6aa2831`). `buildDuelTable` now sums the engine's own stored `points_a`/`points_b` — the quantity migration 121 *defines* `duel_points` as — rather than re-deriving `w * DUEL_WIN + d * DUEL_TIE` from constants held true by a guard test. The season card's copy of the same arithmetic reads the table instead. ⚠ `DuelsTab` still carries **23 `useMemo` blocks** deriving verdicts, form, streaks and movement client-side; that is a bigger question than this row |
| **Aggregating in Node instead of SQL** | the rule settled 2026-07-29 | ✅ **FIXED 2026-09-02** — migration **130** `league_matchweek_points`, returning one row whatever the pool size, breakdown included. ⚠ **Unexecuted; apply before the deploy** |
| **Unbounded reads against the 1,000-row cap** | 155 entries of inflated bonuses; a survey that silently sent to 146 of 3,958 | ✅ **FIXED 2026-09-02.** `readPoolDuels` pages, ordered by `(matchweek_number, duel_id)` so `range()` cannot repeat rows; `readMatchweekPoints` reads no rows at all now. ⚠ **36 other `lib/league/` reads still have no `.range()`** — most are inherently small (20 clubs, 38 matchweeks) and were never the fuse, but nothing asserts that |
...
| # | Change | Size | Why |
|---|---|---:|---|
| ~~1~~ | ✅ **DONE** `6aa2831` — `readPoolDuels` paged and totally ordered; `readMatchweekPoints` no longer reads rows at all (see 2) | — | Two dated bombs. Silent wrong answers, not errors |
| ~~2~~ | ✅ **DONE** `6aa2831` — migration **130**, returns one row whatever the pool size, breakdown included. ✅ **APPLIED to prod 2026-09-02**, verified against real data | — | The rule, applied where we broke it |
| ~~3~~ | ✅ **DONE** `6aa2831` — sums the engine's stored per-duel points, which is what 121 defines `duel_points` as, so it is the same quantity rather than an agreeing re-derivation | — | Removes the second scoring engine |
| ~~4~~ | ✅ **DONE** `ee240bf` — `lib/league/season.ts`, one entry per SEASON, invalidated off the sync's own `changed` array (never a TTL). Measured on prod: the PL season is **165.7 kB shared by 13 pools** | — | 175 kB × every load × every viewer → once per feed write |
| **5** | Scope `readLeaguePoolView` to the tab that needs it | 4 h | ⬜ **Still open, and now the smaller half.** Caching removed the per-viewer cost; this removes the payload. The duel tab needs 1 matchweek, not 38 |
| **6** | Replace the 3 `notify.ts` embeddings with explicit joins | 2 h | 0.5 s/call, before Showdown notifications ship |
| **7** | Retire the remaining `postgres_changes` consumers | 2 d | 🟡 **Half done 2026-09-02** (`8934907`). ⚠ **The measurement reframed it:** writes on replicated tables are 96.5% **two tables** — `user_presence` **63.6%** (web, now **R27**) and `pool_entries` **32.9%** (mobile, now on the broadcast). Three replicated tables have **no consumer at all** and are 0.1% together. ⛔ The publication drop that banks the 32.9% is gated on the **OTA** — land, ship, then drop |

✅ **Items 1–3 landed 2026-09-02** (`6aa2831`), which is the whole *“before Premier League carries load”* block, and **migration 130 is applied and verified in production**. **Items 4–7 are open**, and 4–5 are the
...
**B · ~~Reorder five lines in `app/api/account/delete/route.ts`.~~** ✅ **DONE 2026-09-02** (`6aa2831`)
— **R22**. The guard is first, its error is captured, and `deleteOrder.guard.test.ts` was
negative-tested. **Closes on deploy, not on the commit.** What it does not cover stays open: this
door still hard-deletes rather than retiring (**Gate A2**), and the check-then-act is not
transactional.

**C · ~~The six-hour read-path block.~~** ✅ **DONE 2026-09-02** (`6aa2831`) — **R23** items 1–3, all
three of the ones that silently produce wrong data. ⚠ **Migration 130 is unexecuted and must be
applied before the deploy**, which folds it into step A rather than leaving it as its own step.
...
| **Last Man Standing** | At the **final whistle of each game**, judge that game's pickers | ✅ **FIXED 2026-09-02 — migration 132 applied.** `settle_league_lms_upd` on `league_fixtures` judges at each whistle; the round still closes at matchweek end, gated on `ranks_snapshot_at`, because judging progressively without that gate lets a Saturday result crown a winner whose club plays Monday. Also clears its own verdicts before re-judging, so a corrected result can put somebody back | ✅ **now matches** |
...
✅ **The draw is settled — Decision 13, Ryan 2026-09-02: "only a win keeps you in it."** This confirms
what the code already did and what `verify-last-man-standing.ts` already asserted (*"a draw is not
survival"*), so it needed no change — it needed writing down.
...
✅ **Both shipped in migration 132**, and the second one is why this was never a one-line trigger move.
...
> scoring engines and that they recalculate whenever there is a goal."* Answered against
> **production** (`ujthamlehjyubbzxbnes`) — `pg_trigger`, `pg_proc`, `cron.job` and the data itself,
> not against this document. **Two answers, and neither is yes.**

**Where they run — 6 of 10 are DB-native.**

| Engine | Runs in | On a goal? |
|---|---|---|
| **League fixture** `league_score_fixture` | ✅ Supabase | ✅ **YES, in the database.** Trigger `score_league_fixture_upd` on `league_fixtures` AFTER UPDATE (statement-level, migration 126) → `score_league_fixture_on_change` → `league_score_fixture`. The deployed app *also* calls the RPC, and both are idempotent — belt and braces, not double-scoring |
| **Showdown** `league_score_duels` | ✅ Supabase | ✅ **YES**, by cascade: fixture → `league_finalize_ranks` + `league_snapshot_matchweek_ranks` → `league_matchweeks` UPDATE → `trg_league_settle_duels` |
| **Last Man Standing** `league_lms_settle` | ✅ Supabase | ✅ **YES**, same cascade via `trg_league_settle_lms` |
| **Table mode** `league_score_table` | ✅ Supabase | 🔴 **NO — and it cannot.** See below |
| **Shadow (World Cup)** `shadow_score_match` + 18 more | ✅ Supabase | 🟠 **Not on the goal** — `trg_shadow_bump_inputs_matches` only marks inputs dirty; the scoring runs from pg_cron `shadow-reconcile` (jobid 19, `* * * * *`). Minute cadence, not the whistle |
| **Per-match aggregates** `pool_match_prediction_accuracy` | ✅ Supabase | n/a — `STABLE`, computed on read |
| **Podium** `tournament_podium_view` + `lib/podium.ts` | 🟡 **Both** | with whichever engine calls it |
| **Node "prod" engine** `lib/scoring/core.ts` | 🔴 **Node** | 🟠 Via `api-football-sync` (jobid 8, `* * * * *`) → `recalculatePool({matchId})`. Still enabled: `prod_scoring_enabled = true` |
| **Analytics / XP writer** `lib/analytics/entryAnalytics.ts` | 🔴 **Node** | 🟠 Via `badges.ts` on recalc, plus `analytics-sweep` (jobid 15, `* * * * *` — still **R19**). **No SQL function writes `entry_xp_state`** — verified; 026's two writes are a one-off backfill |
| **Bracket-picker provisional** `lib/bracketPickerScoring.ts` | 🔴 **The browser** | ❌ Recomputed per render, in four `'use client'` components |

**So: the league is genuinely DB-native and goal-driven. The World Cup is not** — it is Node plus a
minute-cadence reconciler, which is exactly what the *Rename: "shadow" → World Cup scoring* and
*Retirement gates* sections already describe, now measured rather than asserted.

#### 🔴 Table mode's score is frozen, and for two pools it is frozen for good

The one finding that is not already in this document, and it is live.
...
function body; it writes `final_rank`, `previous_final_rank`, `updated_at` and nothing else). So the
question is only ever *"what calls `league_score_table`?"* In production, exactly one thing does:

- **The member's own save.** `app/api/pools/[pool_id]/table-prediction/route.ts:150`. The timestamps
  prove it — three pools show `last_scored` within **200 ms** of `last_member_save`.
- **`league_after_standings_change`** would be the right caller, and it exists in the database. It is
  wired to **nothing**: `league_standings` carries **zero triggers**, and its only caller —
  `syncLeagueFixtures.ts:576` — is **not in the deployed 24 Aug code** (`git show
  origin/master:… | grep -c` returns **0**; the same file on master *does* call
  `league_score_fixture` twice, which is why the other three modes work).
- **The `league-standings` cron** is not scheduled. `cron.job` holds no league job of any kind.
  ✅ **2026-09-19: no longer true.** Migration 135 scheduled all four league jobs (reported applied
  via Supabase MCP; `league-outbox` and `league-notices` corroborated from `league_score_events`
  timestamps, `league-standings` and `league-reconcile` awaiting their first scheduled firing).

**The consequence, measured 2026-09-02:** a Table pool's score moves only when somebody edits their
table. After the deadline nobody can, so it stops moving altogether. **Two pools are already there** —
*Predict the Table* (locked 28 Aug) and *Premier League Test Table Prediction* (locked 30 Aug). Their
`league_entry_totals.updated_at` reads 31 Aug 20:59 only because the fixture cascade re-ranked them;
their `bonus_points` has not been recomputed since a member last saved.

⚠ **And the input is stale underneath it.** `league_standings` was last fetched **2026-08-30 18:35**
for every season, with completed fixtures since (1 for the Premier League, 5 for Serie A) — because
nothing refreshes it either. So even the save path now scores against a three-day-old table.

**This is the mode Decision 9 aims at people new to football**, whose whole return reason is *"shown
live against the real table all season"*. It is currently shown against a table that stopped on 30
August. Tracked as **R25**.

#### 🟠 One flag that reads as deliberate and was never inserted

`score_league_fixture_on_change` gates itself on
`sync_settings.league_db_scoring_enabled`, defaulting to `true` when absent — and the row **is
absent**. DB scoring is therefore on, which is what we want, but it is on by fallback rather than by
decision, and **it cannot be switched off without inserting the row first**. Same shape as
`sweep_time_box_enabled` (**V3**), which the *XL→Medium* item called "the last flag to flip" when it
could only be created.

#### What this changes about the answer to "are they all on Supabase"

No. **Three engines are outside the database** and one of those is in the browser. The register
already said so; what the audit adds is that the gap is now **asymmetric by competition** — the
league is fully DB-native and event-driven, the World Cup is Node-plus-cron. That asymmetry is fine
while the World Cup is finished and nothing is scoring, and it becomes the whole problem the moment a
second cup competition runs, because the Node engine is the one with the kickoff CPU spike behind it.
...
- **Is:** The flagship post-WC product — a head-to-head pick'em league. Every matchweek you face one other pool member; beat their accuracy to take the points (3 win / 1 draw / 0 loss, 38-week season). Creates a personal league with named rivals and a "Banter Cup" side-pot. ⚠ **The pairing is a round-robin drawn at pool creation, NOT a weekly random draw** — the May concept's *"randomly paired"* fails gate 5 and was overturned 2026-08-24. Since 2026-08-30 the draw is also **sealed**, opening one matchweek at a time, which is a disclosure choice and not a variance one. Design + mockups: artifact *Showdown Fight Night*.
...
- ⭐ **NO CRON IS INVOLVED IN ANY OF THIS** (110). Every restriction in Table mode — writes closing,
  ...
  > ✅ **2026-09-19 — both are now scheduled** (migration 135: notices hourly, outbox `*/2`). The
  > decision text above is unchanged; only its "still unscheduled" state has moved.
...
- ⚠ **Nothing writes to the database when a clock passes**, so the reveal needs a sweeper.
  ...
  > ⚠ **2026-09-19:** league crons are now scheduled (135), but **none of the four calls
  > `league_sweep_table_reveals`** — the on-read guarantee is still the only one. Unchanged finding,
  > restated so "crons are scheduled" is not read as covering it.
...
- ✅ **135 — APPLIED to production 2026-09-19** (`lib/migrations/135_the_league_crons_are_scheduled.sql`,
  committed `cf1427d` on 6 Sep, held until the routes were deployed per its own header). Schedules
  `league-outbox` `*/2 * * * *`, `league-notices` `0 * * * *`, `league-standings` `0 6 * * 1`,
  `league-reconcile` `0 3 * * *` — the reconcile at 03:00, not the drafted 03:20. Apply is Ryan's
  report via Supabase MCP; `cron.job` is not readable from this session. **Corroborated for two of
  four** from `league_score_events`: the drain at 21:52–21:54 UTC and the even-minute claim cadence
  afterwards (outbox), and notice rows stamped exactly `22:00:01` (notices). The other two fire first
  on 2026-09-20 03:00 and 2026-09-21 06:00. ⚠ The first drain replayed stale events — **R31**.
...
- ✅ **FIXED the same day — `/api/cron/league-reconcile`** (`lib/integrations/apiFootball/
  ...
  it. Suggested `20 3 * * *`.~~ ✅ **Scheduled 2026-09-19** at `0 3 * * *` by migration 135 (see
  above). First scheduled run 2026-09-20 03:00 UTC, unverified until then.
- 🟠 **The placeholder-kickoff strand — found 2026-09-19, fix built, ⛔ NOT DEPLOYED (R30).** The
  22 days this reconcile went unscheduled had a cost the design above did not anticipate: it assumed
  a move is discovered *while the stored kickoff is still ahead*. Providers publish **TBD placeholder
  kickoffs** (La Liga: every Sunday 15:00Z; 35 of its 45 moved rows carried it) and fill the real
  date later — often **earlier** in the week. With no reconcile running, **~202 placeholders across
  five leagues** were never corrected; **111** fixtures in production now hold a kickoff earlier than
  their original. Once the placeholder date passes, no arm can reach the game: the live sync and the
  hourly catch-up (`syncLeagueFixtures.ts` steps 1b/3) both build the provider request from the
  **stored** kickoff, and this reconcile reads only `kickoff_at > now + SYNC_WINDOW_MS`
  (`reconcileLeagueSchedule.ts:127`) by design. Every tick logged `stale=5 unmatched=5` at
  `ok: true` for six days. **Seven fixtures went unscored**: Osasuna–Espanyol, Racing–Alavés, Real
  Madrid–Rayo, Sevilla–Valencia, Athletic–Elche (La Liga MW5, stored 13 Sep, played 11–12 Sep),
  Lazio–AC Milan (stored 13 Sep, played 12 Sep), Monaco–Lens (stored 19 Sep, played 18 Sep).

  **What landed tonight, in order:** (1) 135 applied, reconcile scheduled; (2)
  `run-league-reconcile.ts --apply` by hand — 206 fixtures stamped in the 21:xx UTC hour; (3)
  **`fc23239`** — step 3b in the sync: a stray the day feed did not carry is asked for **by fixture
  id** (`getFixturesByIds`, `client.ts:242`, twenty per call), which knows no date, and the full
  object drops into the ordinary step-6 diff so the real kickoff lands with the score; no call on a
  tick where the day feed carried every stray; run note gains `stray_by_id=fetched/asked`
  (`syncLeagueFixtures.ts:1433`); tests **V3.13–V3.15**, file **60/60**; (4) **`6eb14e4`** —
  `scripts/run-league-sync.ts` had thrown at import since `a3c3e19` (2 Sep) put `server-only` in
  `lib/league/season.ts`; `scripts/lib/no-server-only.cjs` resolves it to nothing, first import;
  (5) the fixed arm run **by hand from the local checkout against production at 22:19 UTC** — all
  seven carry `completed_at = 2026-09-19 22:19:46Z`, and La Liga / Serie A / Ligue 1 standings
  `fetched_at = 22:19:5x`.

  ⛔ **Owed: push + deploy `fc23239`.** Until then the scheduled sync runs the old arm, and the next
  game a provider moves to a day the reconcile has already passed strands the same way — silently.
  The reconcile now catches the *common* case (a move seen while the placeholder is still ahead);
  3b is the only thing that catches the rest. ⚠ Both commits are on local `master`, **109 ahead** of
  `origin/master` tonight.
...
### Decision 11 — A league table is a full-time table

Settled 2026-09-02 with Ryan, after the engine audit found Table mode's score frozen (**R25**).

The question was whether a table pool's score should move **on a goal** or **at full time**. Ryan's
stated expectation was the former — *"based on the score, where they sit in a live table, should move
and therefore recompute your points."*

**The answer is full time, and the reason is the sport, not our architecture.** api-football's
`/standings` is a full-time table; league tables do not update mid-match, anywhere. So a provisional
in-play table is not something we *read* — it is something we would have to **invent**, by overlaying
in-flight fixture deltas onto the ingested standings.
...
- **The overlay is not rejected, it is deferred**, and it is a mode design change rather than a fix.
  ...
  see points deductions* — and treat the overlay as **display-only until full time**, exactly as the
  duel scoreline is live while `duel_points` only pays on settle.
- *Why not just derive it:* a derived table cannot see points deductions, and a provisional result
  that flips back would restate a score a member had already seen. Both are the silent-wrongness class.

### Decision 12 — One read, two surfaces

Settled 2026-09-02 with Ryan, at the point where the RN league build was about to start.

The read-path review's items 4–5 (cache the season read, scope `readLeaguePoolView` to the tab that
needs it) could be done as **web patches** or as a **shared read contract**. Ryan chose the contract.

- **A league surface is read through one server-owned endpoint**, returning what a screen renders and
  nothing else — web and RN call the same thing.
- *Why it is not a preference:* mobile is direct-to-PostgREST, and migration 050's four engine tables
  are deny-all, so RN **cannot read league data at all** without it (**R24**). The contract is not an
...
- ⚠ **The live half must not go through the cache.** Migration 125 already carries score, status and
  minute over the broadcast. Cache the stable season; let the broadcast own what moves.

Plan: `drafts/2026-09-02_table_mode_fix_and_rn_league_plan.md` Part B.

### Decision 13 — Only a win keeps you in

Settled 2026-09-02 with Ryan, walking Last Man Standing's flow.

> **"A draw does not keep you in it. Only a win keeps you in it."**

**This confirms existing behaviour rather than changing it** — `league_lms_settle` already eliminated
on a draw, and `scripts/verify-last-man-standing.ts` already asserted it in as many words (*"pick a
club that DRAWS → out — a draw is not survival"*). What it did not have was a decision behind it.

The full rule, now written down once:

| | |
|---|---|
| Picked club **won** | survives |
| Picked club **drew** | **out** |
| Picked club **lost** | out |
| Picked club had **no completed fixture** that matchweek | survives — *you were not beaten* |
| **No pick** | out |

- *Why it matters enough to record:* at the ~65–70% weekly survival the mode is designed around,
  surviving draws would roughly halve the weekly cull and double how long a round runs. Decision 9
  sized repeating rounds against a single elimination lasting *"five or six matchweeks of
  thirty-eight"*; this is one of the two numbers that sets that length.
- **The generosity is one-directional and stays that way.** A club that did not play survives, because
  the member was not beaten — that is not the same as rewarding a draw, and the two must not be
  collapsed into "anything that isn't a loss".
